...
(() => {
 ...
 queue = (function(): any {
   ...
   function Item( func: Function, self ) {
     this.func = func;
     this.self = self;
     this.next = undefined;
   }
 ...
 function notifyIsolated ( self, callback, chain ): void {
   var ret, _then;
   try {
     if (callback === false) {
 ...
 function iteratePromises ( Constructor, arr, resolver, rejecter ): void {
 ...
 function MakeDeferred ( self ): void {
   this.deferred = self;
   this.triggered = false;
 }

 function Deferred ( self ): void {
   this.promise = self;
   this.state = 0;
   this.triggered = false;
   this.chain = [];
 ...
 function Promise ( executor ): void {
