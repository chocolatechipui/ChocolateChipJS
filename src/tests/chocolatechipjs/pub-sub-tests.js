...
test('$.subscription test.', function(){
   ...
   equal($.isString($.subscriptions['news/update'][0].token), true,'$.subscriptions index token should contain an integer.');
   equal($.isString($.subscriptions['news/update'][0].token), true,'$.subscriptions index token should contain 10 digits.');
...
test('$.publish: push to subscribers.', function(){
   ...
   stop();
   setTimeout(function() {
      equal($.isString($.subscriptions['news/announcement'][0].token), true,'$.subscriptions index token should contain an integer.');
      equal($.subscriptions['news/announcement'][0].token.length, 36,'$.subscriptions index token should contain 10 digits.');
...
test('$.unsubscribe:', function(){
   ...
   stop();
   setTimeout(function() {
      equal($.isString($.subscriptions['news/alerts'][0].token), true,'$.subscriptions index token should contain an integer.');
      equal($.subscriptions['news/alerts'][0].token.length, 36,'$.subscriptions index token should contain 10 digits.');
