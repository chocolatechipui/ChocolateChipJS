...
test('[].appendTo:', function() {
   var item = $('#setEvent');
   var newItem = $.make('<p id="appendedTo">This was appended to the div.</p>').appendTo(item);
...
test('[].prependTo:', function() {
   var item = $('#setEvent');
   var newItem = $.make('<p id="prependTo">This was prepended to the div.</p>').prependTo(item);
   equal($.isArray(newItem), true, 'Should return an array.');
   equal(newItem.length, 1, 'Should return an array with length of 1.');
   equal(newItem[0].nodeName, 'P', 'Should return a new P element.');
...
test('[].html:', function() {
   ...
   equal([].insert().length, 0, 'Should have length of 0.');
   equal($.isArray([].insert()), true, 'Should return an array.');
   equal(list.first().text(), 'a', 'Should be "a".');
   list.first().html('z');
   equal(list.first().text(), 'z', 'Should be "z".');
   var a = list.last().text();
   equal($.isArray(a), false, 'Should not return an array.');
   equal(list.last().children()[0].nodeName, 'SPAN', 'Should return "span".');
   equal(list.last().children().text(), 'c', 'Should return "c".');
   equal(typeof a === 'string', true, 'Should return a string of content.');
   list.children(-2).text('Very Strong!');
   equal(list.children(-2).eq(0).text(), 'Very Strong!', 'Should return "Very Strong!"');
});
// 14
test('[].attr:', function() {
...
test('[].wrap:', function() {
   ...
   equal(wrapper[0].nodeName, 'DIV', 'Should return DIV.');
